import { getWorkOSUserFromReq } from "./workspaceAuth.js";
...
async function authorize(req, res) {
  ...
  if (!user) {
    ...
    return res.redirect(302, `/workspace/login?next=${encodeURIComponent(next)}`);
  }
  ...
  if (consent === "approve") {
    ...
    await recordAnalytics("ai_connection_approved", {
      req,
      brainUser: user.id,
      path: "/oauth/authorize",
      source: client.name || "AI client",
      meta: { scope },
    });
