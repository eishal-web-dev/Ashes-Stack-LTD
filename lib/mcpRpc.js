...
import { bearerFromRequest, mcpResource, verifyMcpAccessToken } from "./mcpAuth.js";
...
async function handleRpc(owner, message) {
  ...
  if (method === "tools/call") {
    const tool = String(message?.params?.name || "unknown").slice(0, 80);
    const started = Date.now();
    try {
      const result = await runTool(owner, tool, message?.params?.arguments || {});
      await recordAnalytics("mcp_tool_call", {
        brainUser: owner,
        source: "MCP",
        path: "/mcp",
        meta: { tool, ok: result?.isError !== true, durationMs: Date.now() - started },
      });
      ...
      await recordAnalytics("mcp_tool_call", {
        ...
        source: "MCP",
        path: "/mcp",
        meta: { tool, ok: false, durationMs: Date.now() - started },
      });
