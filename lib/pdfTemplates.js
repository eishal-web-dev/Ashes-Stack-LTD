...
// pdf-lib's standard fonts use WinAnsi encoding, which can't draw raw
// newline/control characters or most emoji/exotic unicode — attempting to
// draw them throws "WinAnsi cannot encode ...". Strip anything unsafe
// before any drawText call, and keep newlines meaningful by splitting
// paragraphs into separate lines instead of passing them straight through.
...
function addHeader(page, bold, font, label, logo) {
  ...
  const cleanLabel = sanitizeInline(label);
  const labelWidth = font.widthOfTextAtSize(cleanLabel, 8);
  page.drawText(cleanLabel, { x: PAGE_W - MARGIN - labelWidth - 14, y: PAGE_H - 72, size: 8, font: bold, color: GRAY });
  // orange divider bar
  page.drawRectangle({ x: MARGIN, y: PAGE_H - 130, width: PAGE_W - MARGIN * 2, height: 22, color: ORANGE });
}

function addFooter(page, font) {
  page.drawText(sanitizeInline("ASHES STACK  •  SOFTWARE HOUSE  •  CONFIDENTIAL CLIENT DOCUMENT"), {
    x: MARGIN,
    y: 30,
    size: 7,
    font,
...
async function baseDocument({ label, eyebrow, title, subtitle, client, sections, recipientLabel = "CLIENT" }) {
  ...
  let y = PAGE_H - 165;
  page.drawText(sanitizeInline(eyebrow), { x: MARGIN, y, size: 9, font: bold, color: ORANGE });
  y -= 24;
  page.drawText(sanitizeInline(title), { x: MARGIN, y, size: 22, font: bold, color: BLACK });
  y -= 20;
  if (subtitle) {
    page.drawText(sanitizeInline(subtitle), { x: MARGIN, y, size: 10, font, color: GRAY });
    y -= 20;
  }

  // recipient info box
  page.drawRectangle({ x: MARGIN, y: y - 55, width: PAGE_W - MARGIN * 2, height: 55, color: LIGHT });
  page.drawText(sanitizeInline(`${recipientLabel}:  ${client.company || client.name}`), { x: MARGIN + 12, y: y - 20, size: 10, font: bold, color: BLACK });
  page.drawText(sanitizeInline(`CONTACT: ${client.name}  •  ${client.email}`), { x: MARGIN + 12, y: y - 36, size: 9, font, color: GRAY });
  page.drawText(sanitizeInline(`DATE: ${fmtDate(new Date())}`), { x: MARGIN + 12, y: y - 50, size: 9, font, color: GRAY });
  y -= 75;
  ...
  for (const section of sections) {
    ensureRoom(30);
    if (section.heading) {
      page.drawText(sanitizeInline(section.heading), { x: MARGIN, y, size: 13, font: bold, color: BLACK });
      y -= 18;
    }
    if (section.paragraph) {
      // Split on real newlines first (so intentional line breaks the admin
      // typed are preserved), then word-wrap each resulting line.
      const rawLines = sanitizeParagraph(section.paragraph);
      for (const rawLine of rawLines) {
        if (!rawLine) { y -= 14; continue; }
        const wrapped = wrapText(rawLine, font, 10, PAGE_W - MARGIN * 2);
    ...
    if (section.bullets) {
      for (const b of section.bullets) {
        const clean = sanitizeInline(b);
        if (!clean) continue;
        const lines = wrapText(clean, font, 10, PAGE_W - MARGIN * 2 - 14);
    ...
    if (section.table) {
      ...
      headers.forEach((h, i) => {
        page.drawText(sanitizeInline(h), { x: MARGIN + i * colW + 6, y: y - 13, size: 9, font: bold, color: rgb(1, 1, 1) });
      });
      y -= 22;
      rows.forEach((row, ri) => {
        ensureRoom(20);
        ...
        row.forEach((cell, i) => {
          page.drawText(sanitizeInline(String(cell)), { x: MARGIN + i * colW + 6, y: y - 11, size: 9, font, color: BLACK });
        });
        y -= 18;
      });
      y -= 10;
...
export async function generateDocPdf(type, client, meta = {}) {
  switch (type) {
    ...
    case "offer_letter": {
      const offerType = meta.offerType || "Position";
      const article = /^[aeiou]/i.test(offerType) ? "an" : "a";
      const details = [
        ["Position", meta.position],
        ["Offer type", offerType],
        ["Start date", meta.startDate ? fmtDate(meta.startDate) : null],
        ["Duration", meta.duration],
        ["Compensation", meta.compensation],
        ["Location", meta.location],
        ["Reports to", meta.reportsTo],
      ].filter(([, v]) => v);

      const sections = [
        {
          paragraph: `Dear ${client.name},\n\nWe are pleased to offer you the position of ${meta.position || "the role discussed"} as ${article} ${offerType} at ASHES STACK${meta.startDate ? `, commencing ${fmtDate(meta.startDate)}` : ""}. We were impressed by what you bring to the table and are excited about the possibility of you joining us.`,
        },
      ];
      ...
      if (details.length) {
        sections.push({
          ...
          table: { headers: ["FIELD", "DETAIL"], rows: details },
        });
      }

      sections.push({
        heading: "Terms",
        paragraph: meta.additionalTerms || "This offer is subject to mutual agreement on working hours, conduct, and confidentiality of ASHES STACK's business information. Either party may end this arrangement with reasonable notice, consistent with what's discussed and agreed between us.",
      });

      sections.push({
        heading: "Acceptance",
        paragraph: `Please confirm your acceptance of this offer by replying in your ASHES portal${meta.responseDeadline ? ` by ${fmtDate(meta.responseDeadline)}` : ""}. We're looking forward to working with you.\n\nOn behalf of ASHES STACK${meta.issuedByName ? `\n${meta.issuedByName}` : ""}`,
      });

      return baseDocument({
        label: "OFFER LETTER",
        eyebrow: offerType.toUpperCase(),
        title: `Offer of ${offerType}`,
        subtitle: meta.position ? `For the role of ${meta.position}` : undefined,
        // An offer letter addresses the person, never a company name — build a plain
        // object explicitly rather than spreading `client` (a Mongoose document),
        // since spreading a Mongoose doc does NOT copy its schema fields (they sit
        // behind getters, not own-enumerable properties) and silently produces
        // an object with undefined name/email.
        client: { name: client.name, email: client.email, company: undefined },
        recipientLabel: "TO",
        sections,
      });
    }
