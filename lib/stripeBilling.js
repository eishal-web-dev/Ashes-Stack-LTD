...
async function resolveProPriceId() {
  const configuredId = configured("STRIPE_PRO_PRICE_ID");
  ...
  const response = await fetch(`${STRIPE_API}/prices?${query}`, {
    headers: { Authorization: `Bearer ${configured("STRIPE_SECRET_KEY")}` },
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data?.error?.message || "Could not load Stripe prices");
  const monthly = data?.data?.find((price) => price?.recurring?.interval === "month");
  const price = monthly || data?.data?.[0];
  if (!price?.id) throw new Error("This Stripe product has no active recurring price");
  return price.id;
}
...
export async function createCheckoutSession(user) {
  const values = {
    mode: "subscription",
    integration_identifier: "ashes_brain_checkout",
    "line_items[0][price]": await resolveProPriceId(),
