...
const ROBOLAB_AUDIENCE = "ashes-robolab";
...
export function signRoboLabSession(payload) {
  return jwt.sign({ ...payload, typ: "robolab_user" }, ROOT_SECRET, {
    issuer: ISSUER,
    audience: ROBOLAB_AUDIENCE,
    expiresIn: "7d",
  });
}

export function verifyRoboLabSession(token) {
  try {
    const payload = jwt.verify(String(token || ""), ROOT_SECRET, {
      issuer: ISSUER,
      audience: ROBOLAB_AUDIENCE,
    });
    return payload?.typ === "robolab_user" ? payload : null;
  } catch {
    return null;
  }
}
