...
function SignalNetwork(){const canvasRef=useRef<HTMLCanvasElement>(null);useEffect(()=>{const canvas=canvasRef.current;if(!canvas)return;const context=canvas.getContext('2d');if(!context)return;const colors=['#66ebf2','#ff62c7','#d8ff62','#ff875c','#ad77ff'];let width=0,height=0,frame=0,last=0;const pointer={x:-9999,y:-9999};type Node={x:number,y:number,vx:number,vy:number,r:number,color:string,life:number,hot:boolean};let nodes:Node[]=[];
  const reset=(node?:Node)=>{const zone=Math.floor(Math.random()*3);let x:number,y:number,angle:number;if(zone===0){x=width*(.2+Math.random()*.23);y=height*(.2+Math.random()*.28);angle=Math.PI+(-.55+Math.random()*1.1)}else if(zone===1){x=width*(.57+Math.random()*.23);y=height*(.2+Math.random()*.28);angle=-.55+Math.random()*1.1}else{x=width*(.44+Math.random()*.12);y=height*(.53+Math.random()*.2);angle=Math.PI/2+(-.48+Math.random()*.96)}const speed=.72+Math.random()*.95;const next={x,y,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,r:1.1+Math.random()*1.8,color:colors[Math.floor(Math.random()*colors.length)],life:.7+Math.random()*.3,hot:false};if(node)Object.assign(node,next);else return next};
  const resize=()=>{const rect=canvas.getBoundingClientRect();width=rect.width;height=rect.height;const dpr=Math.min(devicePixelRatio,1.5);canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);context.setTransform(dpr,0,0,dpr,0,0);nodes=Array.from({length:width<700?42:86},()=>reset() as Node)};resize();const observer=new ResizeObserver(resize);observer.observe(canvas);
  const move=(event:PointerEvent)=>{const rect=canvas.getBoundingClientRect();pointer.x=event.clientX-rect.left;pointer.y=event.clientY-rect.top};const leave=()=>{pointer.x=-9999;pointer.y=-9999};window.addEventListener('pointermove',move,{passive:true});window.addEventListener('pointerleave',leave);
  const draw=(time:number)=>{if(time-last<32){frame=requestAnimationFrame(draw);return}last=time;context.clearRect(0,0,width,height);for(const node of nodes){node.x+=node.vx;node.y+=node.vy;node.life-=.0023;node.hot=Math.hypot(node.x-pointer.x,node.y-pointer.y)<90;if(node.x< -35||node.x>width+35||node.y< -35||node.y>height+35||node.life<=0)reset(node)}
    const reach=width<700?88:132;for(let i=0;i<nodes.length;i++){for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j],distance=Math.hypot(a.x-b.x,a.y-b.y);if(distance<reach){const hot=a.hot||b.hot;context.beginPath();context.moveTo(a.x,a.y);context.lineTo(b.x,b.y);context.strokeStyle=hot?`rgba(220,250,255,${(1-distance/reach)*.9})`:`rgba(153,225,232,${(1-distance/reach)*.26*Math.min(a.life,b.life)})`;context.lineWidth=hot?1.5:.65;context.shadowBlur=hot?12:0;context.shadowColor=hot?a.color:'transparent';context.stroke();context.shadowBlur=0}}}
    for(const node of nodes){context.beginPath();context.arc(node.x,node.y,node.hot?node.r*2.15:node.r,0,Math.PI*2);context.fillStyle=node.hot?'#ffffff':node.color;context.globalAlpha=node.hot?1:Math.min(1,node.life*1.8);context.shadowBlur=node.hot?28:12;context.shadowColor=node.color;context.fill();context.globalAlpha=1;context.shadowBlur=0}frame=requestAnimationFrame(draw)}
  if(!matchMedia('(prefers-reduced-motion: reduce)').matches)frame=requestAnimationFrame(draw);return()=>{cancelAnimationFrame(frame);observer.disconnect();window.removeEventListener('pointermove',move);window.removeEventListener('pointerleave',leave)}},[]);return <canvas ref={canvasRef} className="signal-network" aria-hidden="true"/>}
