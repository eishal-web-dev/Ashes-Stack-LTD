...
export default function App(){const [loaded,setLoaded]=useState(false);const [power,setPower]=useState(0);useEffect(()=>{const t=setTimeout(()=>setLoaded(true),700);return()=>clearTimeout(t)},[]);return <>
  <div className={`loader ${loaded?'gone':''}`}><span>ASHES</span><i/></div><div className="grain"/><Nav/>
  <main>
    <section id="top" className="hero"><div className="ash-field" aria-hidden="true">{Array.from({length:28},(_,i)=><i key={i}/>)}</div><div className="radar radar-a"/><div className="radar radar-b"/><Phoenix className="hero-phoenix" priority/><div className="hero-energy"/>
      ...
      <div className="rotate"><span/><p>DRAG TO<br/>ROTATE</p></div>
    </section>
    ...
    <section id="work" className="work"><Reveal><p className="kicker">SELECTED WORK.</p><div className="title-row"><h2>STUFF WE’VE<br/>BROUGHT TO LIFE.</h2><p>MOVE TO EXPLORE</p></div></Reveal><div className="work-stage"><Phoenix className="work-bird"/><div className="work-cards">
    ...
    <section className="philosophy"><Reveal><p className="kicker">OUR PHILOSOPHY</p><h2>SERIOUS ENGINEERING.<br/>UNREASONABLY GOOD DESIGN.</h2><p>BUILT TO WORK. MADE TO HIT DIFFERENT.</p></Reveal><Phoenix className="philosophy-bird"/><div className="principles"><div><Crosshair/><span><b>THINK SHARP</b>Curious minds.<br/>Clear thinking.</span></div><div><Box/><span><b>BUILD SOLID</b>Clean code.<br/>Strong foundations.</span></div><div><span className="spark">✳</span><span><b>MAKE IT ICONIC</b>Details that click.<br/>Design that lasts.</span></div></div></section>

    <section className="process"><Reveal><p className="kicker">OUR PROCESS</p><h2>FROM FIRST PRINCIPLE<br/>TO FINAL PIXEL.</h2></Reveal><div className="process-world"><Phoenix className="process-left"/><div className="process-core"/><Phoenix className="process-right"/><div className="process-grid">{['VIBE CHECK','MAKE IT\nMAKE SENSE','BUILD\nTHE THING','SHIP &\nEVOLVE'].map((x,i)=><div key={x}><span>0{i+1}</span><b>{x.split('\n').map(v=><em key={v}>{v}</em>)}</b><small>{['Discover','Design','Engineer','Improve'][i]}</small></div>)}</div></div></section>

    <section id="studio" className="about"><Reveal><p className="kicker">ABOUT ASHES</p><h2>ASHES IS A DESIGN-LED<br/>SOFTWARE HOUSE FOR<br/>BRANDS THAT REFUSE<br/>TO BE BORING.</h2></Reveal><ul><li>◇ THREE.JS</li><li>⚛ REACT</li><li>⬡ AI & COMPUTER VISION</li><li>▧ FULL-STACK</li><li>◎ PRODUCT DESIGN</li></ul><Phoenix className="about-bird"/></section>

    <section className="cta"><Phoenix className="cta-bird" variant="vr"/><div><h2>GOT A WILD IDEA?<br/>GOOD. WE LIKE THOSE.</h2><a href="mailto:hello@ashes.studio">LET'S BUILD IT <ArrowUpRight/></a></div></section>
