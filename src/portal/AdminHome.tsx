...
export default function AdminHome() {
  ...
  async function onCreate(e: FormEvent) {
    ...
    setSuccess(`Account created for ${data.email} (${data.role}).${data.emailSent ? ' Login details emailed to them.' : ' Email not configured — share these login details with them directly.'}`);
  ...
  if (loading) return <AdminLayout user={user}><div>Loading…</div></AdminLayout>;
    ...
    <AdminLayout user={user}>
      <div className="portal-page-head">
        ...
        <p className="portal-sub">
          Click into a client to send an invoice, contract, or any other document in one click.
        </p>
      </div>
      ...
      <div className="portal-card">
        ...
        {clients.length === 0 ? (
          ...
          <table className="portal-table">
            <thead><tr><th>Name</th><th>Email</th><th>Company</th><th>Stage</th><th>Docs sent</th><th></th></tr></thead>
            <tbody>
              {filtered.map((c) => (
                <tr key={c._id}>
                  <td>{c.name}</td>
                  <td>{c.email}</td>
                  <td>{c.company}</td>
                  <td>{c.stage ? <span className="portal-badge sent">{STAGE_LABELS[c.stage] || c.stage}</span> : '—'}</td>
                  <td>{c.docCount}</td>
